    ...
    fun parse(d: String): PathData {
        ...
        while (i < tokens.size) {
            ...
            when (cmd) {
                ...
                'H' -> {
                    current = Point(num(), current.y)
                    path = path.lineTo(current)
                }
                'h' -> {
                    current = Point(current.x + num(), current.y)
                    path = path.lineTo(current)
                }
                'V' -> {
                    current = Point(current.x, num())
                    path = path.lineTo(current)
                }
                'v' -> {
                    current = Point(current.x, current.y + num())
                    path = path.lineTo(current)
                }
                'C' -> {
                    val cp1 = point()
